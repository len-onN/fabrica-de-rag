...
## 2026-07-02 - Estabilização da Suíte E2E e Navegação Local

Após a resolução da infraestrutura do container Dev, surgiram sintomas pontuais tanto na execução real quanto nos testes E2E do projeto. O dashboard entrava em loop infinito de carregamento (race condition com o RxJS e AuthState) e a interface mascarava os erros da API devido ao `eventCoalescing` ativo no Angular 18, resultando na impressão de problemas que já haviam sido corrigidos. Além disso, a navegação principal (Coleções) não estava ancorada no menu, impossibilitando que os usuários criassem coleções e usassem a Fábrica a partir de uma conta nova.

A correção abrangeu toda a pilha: no Spring Boot, o `GlobalExceptionHandler` passou a converter `IllegalArgumentException` e outros erros de negócio puramente semânticos para o status 400 Bad Request, ao invés de estourar 500 no log e silenciar a UI. No Angular, usamos o `ChangeDetectorRef.detectChanges()` explicitamente no login para garantir o feedback instantâneo da captura de erros HTTP, remediando a dessincronização do form control. Inverteu-se a ordem de subscrição no `DashboardComponent` para evitar que o sinal emitisse antes dos listeners. A barra lateral `shell.component.ts` ganhou a rota oficial para Coleções, ativando as jornadas de fato do MVP.

Na automação de testes E2E com Playwright, a estabilidade estava sendo afetada pela concorrência nos testes: os workers conflitavam ao usar a mesma base de dados. A estratégia adotada foi desabilitar o `fullyParallel: false` em `playwright.config.ts`, permitindo que o hook global isolasse adequadamente a conta rodada. Adicionalmente, quebramos os fluxos em arquivos distintos (`auth.spec.ts` para telas não autenticadas e `mvp-flow.spec.ts` para navegações e fluxos autênticos do RAG), o que facilitou a compreensão dos relatórios de teste sem interferir negativamente no budget das pipelines contínuas locais.

Com essas validações o projeto segue com 100% de estabilidade nos testes E2E e fica apto para o Commit, abrindo a porta para a documentação técnica oficial ou features avançadas.
