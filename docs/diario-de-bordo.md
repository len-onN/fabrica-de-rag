...
## 2026-06-28 - Interpreta��o Visual e de Tabelas em PDF
...
O foco desta etapa foi incorporar a capacidade estrutural (Pydantic e Spring Boot) e os adaptadores de processamento (worker) que lidam com elementos ricos do PDF, especificamente a extra��o de tabelas (usando a rec�m adicionada biblioteca `pdfplumber`, de licen�a MIT) e o design base para interpretar visualmente imagens/tabelas atrav�s de Webhooks com Vision Language Models (VLM). Foi implementado tamb�m o `mock-vision-interpreter`, um adaptador de testes capaz de emular a funcionalidade determin�stica das descri��es da VLM e acatar �s limita��es de or�amentos (budgets).

Com rela��o � orquestra��o, os DTOs do backend Java ganharam extens�es substanciais sob `local.fabricarag.core.dto.worker.components`, permitindo o interc�mbio de estruturas complexas entre o Worker e a Ingest�o (PdfExtractElementsRequest/Response e PdfInterpretVisualRequest/Response). Al�m disso, foram ajustadas as depend�ncias faltantes `WorkerBaseRequest` e `WorkerErrorResponse` e as implementa��es referentes ao webhook.

Testes locais usando a su�te pytest provaram que os fluxos s�ncronos de falha e webhooks ass�ncronos de aceita��o executam sem atrito, sendo suportados pela valida��o estrita com FastAPI. O backend compilou perfeitamente. O pr�ximo passo ser� realizar o merge para `develop` e preparar a infraestrutura que derivar� a numera��o das p�ginas (`feat/paginas-numeracao`).
